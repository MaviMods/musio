...
export async function homePageData(language) {
  try {
    ...
    const data = await loadTracks(`ytsearch:top hits ${lang}`);
...
function mapTrack(t) {
  ...
  const artworkUrl = t.info?.artworkUrl ?? "";
  ...
  return {
    ...
    playCount: null,
    // image array — all three slots point to same artwork
    image: [
      { url: artworkUrl },
      { url: artworkUrl },
      { url: artworkUrl },
    ],
    // downloadUrl — route through /api/stream proxy using encoded track
    // Player.jsx checks index [0] first, fallback [4]; fill all slots
    downloadUrl: (() => {
      const streamUrl = t.encoded
        ? `/api/stream?encoded=${encodeURIComponent(t.encoded)}`
        : uri;
      return [
        { url: streamUrl },
        { url: streamUrl },
    ...
    primaryArtistsId: "",
    // artists.primary array shape used by Track.jsx and FullscreenTrack.jsx
    artists: {
      primary: author
        ? author.split(",").map((a, i) => ({ id: String(i), name: a.trim() }))
        : [],
    },
    // album shape used by Player.jsx mediaMetaData
    album: { id: "", name: "" },
    encoded: t.encoded ?? "",
  };
}
  ...
  try {
    ...
    if (Array.isArray(id)) {
      const results = await Promise.all(
        id.map(async (songId) => {
          const data = await loadTracks(`ytsearch:${songId}`);
    ...
    // Use ytsearch: prefix — more reliable than direct URL with youtube plugin
...
export async function getAlbumData(id) {
  try {
    const data = await loadTracks(`ytsearch:${id}`);
...
export async function getArtistSongs(id, page = 0) {
  try {
    const data = await loadTracks(`ytsearch:${id} songs`);
...
export async function getArtistAlbums(id, page = 0) {
  try {
    const data = await loadTracks(`ytsearch:${id} albums`);
    const tracks = data?.data ?? data?.tracks ?? [];
    return tracks.slice(page * 10, page * 10 + 10);
  } catch (error) {
...
export async function getSearchedData(query) {
  try {
    const data = await loadTracks(`ytsearch:${query}`);
...
export async function getRecommendedSongs(artistId, songId) {
  try {
    const data = await loadTracks(`ytsearch:${artistId} recommended`);
    return data?.data ?? data?.tracks ?? [];
  } catch (error) {
    console.log(error);
  }
